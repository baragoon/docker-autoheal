...
listenToDockerEvents()
{
  ...
  do
    echo "$LOGLINE"
    [[ $LOGLINE != *"com.docker.compose.project=$COMPOSE_PROJECT_NAME"* ]] && continue
    ...
    [[ $LOGLINE == *"com.docker.compose.service=should-keep-restarting "* && $keep_restarting_seen -eq 0 ]] && echo "OK: Expected restart on should-keep-restarting container!" && keep_restarting_seen=1 && expected_restarts=$((expected_restarts + 1))
    [[ $LOGLINE == *"com.docker.compose.service=should-keep-restarting-stopped "* && $keep_restarting_stopped_seen -eq 0 ]] && echo "OK: Expected restart on should-keep-restarting-stopped container!" && keep_restarting_stopped_seen=1 && expected_restarts=$((expected_restarts + 1))
